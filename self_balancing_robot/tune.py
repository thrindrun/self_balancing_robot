...
def notification_handler(sender, data):
    ...
    try:
        ...
        # GÜNCELLEDİĞİMİZ KISIM: "Ang" ve "PWM" değerlerini yakalar
        # Desen: Ang: (sayı), PWM: (sayı)
        match = re.search(r"Ang:\s*([-\d.]+),\s*PWM:\s*([-\d.]+)", text)
...
def input_thread():
    """Reads terminal input for LQR tuning."""
    print("=" * 50)
    print("LQR Tuning Interface Ready")
    print("Format: <P|Q|R|S> <value>")
    print("P: k1, Q: k2, R: k3, S: k4")
    print("Example: P -70.5")
    print("Commands: S (Start), X (Stop)")
    print("=" * 50)
    
    # Kabul edilen komut harfleri listesi
    valid_params = ['P', 'Q', 'R', 'S']
    ...
    while True:
        try:
            ...
            # Sistem başlatma/durdurma komutları
            if cmd in ['S', 'X']:
                send_queue.put(cmd)
                continue
                
            parts = cmd.split()
            # P, Q, R veya S parametrelerini kontrol et
            if len(parts) == 2 and parts[0] in valid_params:
                try:
                    val = float(parts[1])
                    # ESP32 tarafı "P-70.5" gibi bir string bekliyor
                    msg = f"{parts[0]}{val}"
                    send_queue.put(msg)
                except ValueError:
                    print("Hata: Geçersiz sayı formatı. Örnek: Q -50.2")
            else:
                print("Geçersiz komut! S/X (Başlat/Durdur) veya P, Q, R, S katsayılarını kullanın.")
